...
import type { TransaksiMasuk } from "../../types";
import type { ColumnsType } from "antd/es/table";
import { formatDate } from "../../utils/formatters";
import dayjs from "dayjs";

import TransaksiMasukModal from "./TransaksiMasukModal";
...
const TransaksiMasukList: React.FC = () => {
  ...
  const [selectedTransaksi, setSelectedTransaksi] =
    useState<TransaksiMasuk | null>(null);
  ...
  const handleViewDetails = async (record: TransaksiMasuk) => {
    try {
      const fullData = await transaksiMasukApi.getById(
        record.id_transaksi_masuk
      );
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        // Only update header fields, details editing not supported in this simple view
        ...
        const formattedValues = {
          ...
          details:
            values.details?.map((d: any) => ({
              ...d,
              tanggal_kadaluarsa: d.tanggal_kadaluarsa
                ? d.tanggal_kadaluarsa.format("YYYY-MM-DD")
                : null,
            })) || [],
        };
        await transaksiMasukApi.create(formattedValues);
        message.success("Transaksi masuk berhasil dibuat");
      }
          ...
          <div>
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col span={12}>
                ...
                {formatDate(selectedTransaksi.tanggal_masuk)}
              </Col>
              <Col span={12}>
                <strong>Supplier:</strong>{" "}
                {selectedTransaksi.nama_supplier || "-"}
              </Col>
